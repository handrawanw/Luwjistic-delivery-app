...
class User {
    ...
    static Daftar(req,res,next){
        const { username, password, fullname, role_daftar, role, email, alamat_kurir, status_pernikahan, no_hp, kebangsaan } = req.body;
        UsersModel.findOne({email}).then(async (Users) => {
            if(Users){
                ...
                if (Account) {
                    ...
                    if(role_daftar&&Number(role_daftar)===0){// jika role daftarnya 0 maka dia kurir dan harus isi biodata kurir
                        await Kurir.create({user:Account._id ,alamat_kurir, status_pernikahan, no_hp, kebangsaan })
                    }
                    /*
